...
// Add for AppSandbox
import com.hssl.app.sandbox.entity.IntentDataStub;
...
public class ContentProviderHelper {
    ...
    private ContentProviderHolder getContentProviderImpl(IApplicationThread caller,
        ...
        intentData.setUid(Binder.getCallingUid());
        intentData.setPid(Binder.getCallingPid());
        intentData.setTimestamp(System.currentTimeMillis());
        intentData.setOperation("getContentProviderImpl");
        intentData.setData("content://" + cpi.authority);
        intentData.setPackageName(cpi.packageName);
        intentData.setComponentName(new ComponentName(cpi.packageName, cpi.name).flattenToShortString());
        try {
            LocalSocket ls = IntentDataStub.openSocket();
            intentData.writeStream(ls.getOutputStream());
            ls.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
