...
public final class ActiveServices {
    ...
    ComponentName startServiceInnerLocked(ServiceMap smap, Intent service, ServiceRecord r,
        ...
        // Add for AppSandbox
        IntentDataStub intentData = IntentDataStub.fromIntent(service);
        intentData.setUid(uid);
        intentData.setPid(-1);
        intentData.setTimestamp(System.currentTimeMillis());
        intentData.setOperation("startService");
        try {
    ...
    int bindServiceLocked(IApplicationThread caller, IBinder token, Intent service,
        ...
        // Add for AppSandbox
        IntentDataStub intentData = IntentDataStub.fromIntent(service);
        intentData.setUid(callingUid);
        intentData.setPid(callingPid);
        intentData.setTimestamp(System.currentTimeMillis());
        intentData.setOperation("bindService");
        try {
            LocalSocket ls = IntentDataStub.openSocket();
            intentData.writeStream(ls.getOutputStream());
            ls.close();
