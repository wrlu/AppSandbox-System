...
#define IF_USE_APP_SANDBOX() if (true)
    ...
    case BR_TRANSACTION:
        {
            ...
            // Log to app sandbox
            IF_USE_APP_SANDBOX() {
                if (hssl::AppSandbox::filterCaller(tr.sender_euid, tr.sender_pid) &&
                        hssl::AppSandbox::filterReceiver(getuid(), getpid())) {
                    hssl::AppSandbox::sendThroughSocket(tr.code, buffer, nullptr,
                        tr.sender_euid, tr.sender_pid,
                        hssl::BinderData::BR_TRANSACTION);
                }
            }
            if (tr.target.ptr) {
