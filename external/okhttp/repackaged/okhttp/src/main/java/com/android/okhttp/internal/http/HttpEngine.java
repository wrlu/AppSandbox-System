...
public final class HttpEngine {
  ...
  public static final int MAX_LOCAL_BODY = 100000;
  public static final long MAX_BUFFER_BODY = 100*1000*1000;
  ...
  // Modifications here
  public HttpDataStub currResponseData = new HttpDataStub();
  public boolean okOutput = false;
  public Closeable localSocket;
  ...
  public void sendRequest() throws RequestException, RouteException, IOException {
    ...
    if (networkRequest != null) {
      ...
      if (callerWritesRequestBody && permitsRequestBody(networkRequest) && requestBodyOut == null) {
        ...
        if (bufferRequestBody) { //说明这个请求包除了headers还有body
          ...
          if (contentLength != -1) {
            ...
            httpStream.writeRequestHeaders(networkRequest); //写入headers
            requestBodyOut = new RetryableSink((int) contentLength); //在sendRequests中才第一次建立了这个requestBodyOut变量，用于body数据的写入
          } else {
  ...
  private Response unzip(final Response response) throws IOException {
    ...
    //如果包含gzip，则重建source为GzipSource，并且重构整个response
    //根据http的规则会把Content-Encoding和Content-Length字段删除
    GzipSource responseBody = new GzipSource(response.body().source(), this);
  ...
  public void readResponse() throws IOException {
    ...
    //从标准http进入的话，initHttpEngine是不会进入前面的两个分支的。
    if (forWebSocket) {
      httpStream.writeRequestHeaders(networkRequest);
      ...
      if (sentRequestMillis == -1) { //如果由于Content-Length还不知道，之前没有发过headers，现在进行补发。【添加了header字段】
      ...
      //从这里开始打点记录request？【但是这也意味着，对于不申请respond的request不能进行单独记录】
      //Modifications here
      Request recordRequest = networkRequest;
      HttpDataStub currRequestData = new HttpDataStub();
      long timeStamp = System.currentTimeMillis();
      currRequestData.setTimestamp(timeStamp);
      currRequestData.setUid(HttpDataStub.myUid());
      currRequestData.setPid(HttpDataStub.myPid());
      StringBuilder sb = new StringBuilder();
      sb.append(recordRequest.method());
      sb.append(" ");
      sb.append(recordRequest.urlString());
      sb.append(" ");
      if (httpStream instanceof Http1xStream){
        sb.append("http/1.1");
      }else if(httpStream instanceof Http2xStream){
        sb.append("http/2.0");
      } else {
        sb.append("http/unknown");
      }
      currRequestData.setRequestOrStatusLine(sb.toString());
      List<String> strHeaders = new ArrayList<>();
      for (int i = 0, size = recordRequest.headers().size(); i < size; i ++) {
        strHeaders.add(recordRequest.headers().name(i) + ": " + recordRequest.headers().value(i));
      }
      currRequestData.setHeaderLines(strHeaders);
      Closeable ls = HttpDataStub.openSocket();
      if (ls != null) {
        currRequestData.writeBasicInfo(HttpDataStub.getOutputStream(ls));
        currRequestData.writeHeader(HttpDataStub.getOutputStream(ls));
      }

      //现在只需要发送requestBody即可
      ...
      if (requestBodyOut != null) {
        ...
        if (requestBodyOut instanceof RetryableSink) {
          if (ls != null) {
            byte[] tmpBytes = ((RetryableSink) requestBodyOut).readToBytes();
            HttpDataStub.writeBody(HttpDataStub.getOutputStream(ls), tmpBytes, 0, tmpBytes.length);
          }
          httpStream.writeRequestBody((RetryableSink) requestBodyOut);
        }
      }
      //call request recording function
      if (ls != null) {
        ls.close();
      }
      networkResponse = readNetworkResponse();
    ...
    if (cacheResponse != null && validate(cacheResponse, networkResponse)) { //如果有cache response会进行merge
    ...
    currResponseData.setUid(HttpDataStub.myUid());
    currResponseData.setPid(HttpDataStub.myPid());
    long timeStamp2 = System.currentTimeMillis();
    currResponseData.setTimestamp(timeStamp2);
    StringBuilder sb2 = new StringBuilder();
    sb2.append(userResponse.protocol().toString());
    sb2.append(" ");
    sb2.append(String.valueOf(userResponse.code()));
    sb2.append(" ");
    sb2.append(userResponse.message());
    currResponseData.setRequestOrStatusLine(sb2.toString());
    List<String> responseHeaders = new ArrayList<String>();
    for (int i = 0, size = userResponse.headers().size(); i < size; i ++) {
      responseHeaders.add(userResponse.headers().name(i) + ": " + userResponse.headers().value(i));
    }
    currResponseData.setHeaderLines(responseHeaders);
    if (!this.okOutput && this.localSocket != null) {
      this.currResponseData.writeBasicInfo(HttpDataStub.getOutputStream(this.localSocket));
      this.currResponseData.writeHeader(HttpDataStub.getOutputStream(this.localSocket));
    }
  }
