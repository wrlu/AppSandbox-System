...
public final class GzipSource implements Source {
  ...
  private HttpEngine tmpHttpEngine;
  //如果在Content-Encoding中包含gzip，则会建立Gzip Source

  public GzipSource(Source source, HttpEngine httpEngine) {
    ...
    //Modifications here
    try {
      httpEngine.okOutput = false;
      httpEngine.localSocket = HttpDataStub.openSocket();
    } catch (IOException e) {
        e.printStackTrace();
    } 
    this.tmpHttpEngine = httpEngine;
  }
  ...
  @Override public long read(Buffer sink, long byteCount) throws IOException {
    ...
    if (section == SECTION_BODY) { //读取真实的压缩内容
      ...
      if (result != -1) {
        ...
        //Modifications here
        if(!tmpHttpEngine.okOutput && tmpHttpEngine.localSocket != null){
          HttpDataStub.writeBody(HttpDataStub.getOutputStream(tmpHttpEngine.localSocket), sink.clone().readByteArray(), 0, (int)result);
        }else{
          throw new IllegalStateException("unexcepted bodyrecording status");
        }
        return result;
      }
      section = SECTION_TRAILER;  //只有读取到尾部的时候，即result返回-1，才会转换到SECTION_TRAILER状态
    }
  ...
  private void consumeHeader() throws IOException {
    ...
    // | CRC16 |
    // +---+---+
    if (fhcrc) {  //对file header进行crc check
  ...
  @Override public void close() throws IOException {
    if(!tmpHttpEngine.okOutput && tmpHttpEngine.localSocket != null) {
      tmpHttpEngine.localSocket.close();
      tmpHttpEngine.okOutput = true;
    }
